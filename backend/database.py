...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS ip_reputation (
            ...
            kaspersky_zone  TEXT,   -- Red / Orange / Yellow / Grey / Green (Kaspersky OpenTIP)
            raw_kaspersky   TEXT    -- full JSON from Kaspersky OpenTIP
        );
    ...
    # Migration for DBs created before Kaspersky OpenTIP support was added —
    # CREATE TABLE IF NOT EXISTS above doesn't add columns to an existing table.
    existing_cols = {row["name"] for row in
                      conn.execute("PRAGMA table_info(ip_reputation)").fetchall()}
    for col, coltype in (("kaspersky_zone", "TEXT"), ("raw_kaspersky", "TEXT")):
        if col not in existing_cols:
            conn.execute(f"ALTER TABLE ip_reputation ADD COLUMN {col} {coltype}")
    conn.commit()
...
def upsert_ip_reputation(ip: str, geo: dict = None, abuse: dict = None, kaspersky: dict = None):
    with tx() as c:
        ...
        if kaspersky:
            updates.update({
                "kaspersky_zone": kaspersky.get("Zone"),
                "raw_kaspersky":  json.dumps(kaspersky),
            })
