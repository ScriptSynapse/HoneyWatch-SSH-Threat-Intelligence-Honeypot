...
ABUSEIPDB_KEY  = os.environ.get("ABUSEIPDB_KEY", "")        # https://www.abuseipdb.com/api  (free tier: 1000/day)
KASPERSKY_KEY  = os.environ.get("KASPERSKY_OPENTIP_KEY", "")  # https://opentip.kaspersky.com/token (free)
SHODAN_KEY     = os.environ.get("SHODAN_KEY", "")            # https://developer.shodan.io    (optional)
GEO_CACHE_TTL  = 86400       # 24 hours
ABUSE_CACHE_TTL = 3600 * 6   # 6 hours
KASPERSKY_CACHE_TTL = 3600 * 6   # 6 hours — OpenTIP free tier has a daily request quota

# OpenTIP "Zone" -> a 0-100 score on the same scale as AbuseIPDB's abuseConfidenceScore,
# so it can feed the existing abuse_score column / dashboard colors / alert thresholds
# without a second scoring system to reason about.
KASPERSKY_ZONE_SCORE = {
    "Red":    95,   # Dangerous
    "Orange": 75,   # Not trusted / may host malicious objects
    "Yellow": 40,   # Adware and other
    "Grey":   0,    # No data
    "Green":  0,    # Good / no threats detected
}
...
def get_kaspersky(ip: str) -> dict:
    """Check Kaspersky OpenTIP for ip reputation (if key configured)."""
    if not KASPERSKY_KEY or _is_private(ip):
        return {}

    row = db.query("SELECT * FROM ip_reputation WHERE ip=?", (ip,))
    if row and row[0].get("raw_kaspersky"):
        cached_at = row[0].get("last_checked", "")
        age = (datetime.now(timezone.utc) - _parse_ts(cached_at)).total_seconds()
        if age < KASPERSKY_CACHE_TTL:
            return json.loads(row[0]["raw_kaspersky"])

    try:
        r = requests.get(
            "https://opentip.kaspersky.com/api/v1/search/ip",
            headers={"x-api-key": KASPERSKY_KEY, "Accept": "application/json"},
            params={"request": ip},
            timeout=6,
        )
        if r.status_code == 200:
            ...
            log.warning("Kaspersky OpenTIP: invalid API key (401)")
            data = {}
        elif r.status_code == 403:
            log.warning("Kaspersky OpenTIP: quota/rate limit exceeded (403)")
            data = {}
        else:
            data = {}
    except Exception as e:
        log.debug("Kaspersky OpenTIP lookup failed for %s: %s", ip, e)
        data = {}

    if data:
        db.upsert_ip_reputation(ip, kaspersky=data)
    return data


def kaspersky_score(data: dict) -> int:
    """Map an OpenTIP response's Zone to a 0-100 score, matching AbuseIPDB's scale."""
    return KASPERSKY_ZONE_SCORE.get((data or {}).get("Zone"), 0)
...
def _enrich_worker():
    while True:
        ...
        try:
            geo       = get_geo(ip)
            abuse     = get_abuse(ip)
            kaspersky = get_kaspersky(ip)

            abuse_conf   = abuse.get("abuseConfidenceScore", 0)
            kasp_score   = kaspersky_score(kaspersky)
            score        = max(abuse_conf, kasp_score)   # worst-of-both, same 0-100 scale

            if score and score > 50:
                source = "Kaspersky" if kasp_score >= abuse_conf else "AbuseIPDB"
                alert(f"High-risk IP {ip} connected (score {score}, {source})", "high")

            # ip_reputation.abuse_score is what the dashboard's IP table / map
            # actually reads (see ws_server.py) — keep it in sync with the
            # combined score, not just whichever source last wrote it.
            db.get_conn().execute(
                "UPDATE ip_reputation SET abuse_score=? WHERE ip=?", (score, ip)
            )
